...
int wpa_sta_add_format(char *pdu, int *p_size,struct hostapd_sta_add_params *params)
{
	...
	if(params->supp_rates_len) {
		len = sizeof(element->len) + params->supp_rates_len;
		if(pdu_size < counter + len)
		{
		...
		element = (struct wiflow_pdu_element *)(pdu + counter);
		element->len = params->supp_rates_len;
	...
	counter += len;
	/*set*/
	len = sizeof(element->len) + sizeof(params->set);
	if(pdu_size < counter + len)
	{
		fprintf(stderr,"set Error,%s:%d\n",__FILE__,__LINE__);
		goto err;
	}
	element = (struct wiflow_pdu_element *)(pdu + counter);
	element->len = sizeof(params->set);
	memcpy(&element->data,&params->set,element->len);
	counter += len;
	/*qosinfo*/
...
int wpa_sta_add_parser(char * pdu,int p_size,struct hostapd_sta_add_params * params)
{
	...
	element = (struct wiflow_pdu_element *)(pdu + counter);
	memcpy(&params->supp_rates_len,&element->data, element->len);
	counter += len;
	/*supp_rates*/
	if(params->supp_rates_len) {
		len = sizeof(element->len) + params->supp_rates_len;
		...
		element = (struct wiflow_pdu_element *)(pdu + counter);
		p = malloc(params->supp_rates_len);
		memcpy(p,&element->data,params->supp_rates_len);
	...
	element = (struct wiflow_pdu_element *)(pdu + counter);
	memcpy(&params->set,&element->data,sizeof(params->set));
	counter += len;
	/*qosinfo*/
	len = sizeof(element->len) + sizeof(params->qosinfo);
	if(p_size < counter + len)
...
int wpa_set_ap_parser(char * pdu,int pdu_size, struct wpa_driver_ap_params * params)
{
	...
	element = (struct wiflow_pdu_element *)(pdu + counter);
	memcpy(&params->ssid_len,&element->data,t_size);
	counter += len;
	/*ssid*/
...
int wpa_set_frag_parser(char * pdu, int pdu_size, int *frag)
{
	...
	memcpy(frag,&element->data,element->len);
	return 0;
...
int remote_hw_modes_parser(char * pdu, int pdu_size, struct hostapd_hw_modes *remote_hw_modes)
{
	...
	memcpy(&remote_hw_modes->num_rates,&element->data,element->len);
	counter += len;
	/* rates*/
	element = (struct wiflow_pdu_element *)(pdu + counter);
	len = sizeof(element->len) + element->len;
	...
	remote_hw_modes->rates = malloc(element->len);
	memcpy(remote_hw_modes->rates,&element->data,element->len);
	counter += len;
	/* others*/
	element = (struct wiflow_pdu_element *)(pdu + counter);
	len = sizeof(element->len) + element->len;
...
int wpa_supplicant_data_format(char *pdu, int *p_size, union wpa_event_data *data, enum wpa_event_type *event)
{
	...
	switch(*event)
	{
		...
		case EVENT_TX_STATUS:
			...
			/*type*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			element->len = sizeof(data->tx_status.type);
			len = sizeof(element->len) + element->len;
			if(pdu_size < counter + len)
			{
				goto err;
			}
			memcpy(&element->data, &data->tx_status.type, element->len);
			counter += len;
			/*dst*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			element->len = ETH_ALEN;
			...
			/*stype*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			element->len = sizeof(data->tx_status.stype);
			len = sizeof(element->len) + element->len;
			if(pdu_size < counter + len)
			{
				goto err;
			}
			memcpy(&element->data, &data->tx_status.stype, element->len);
			counter += len;
			/*ack*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			element->len = sizeof(data->tx_status.ack);
			len = sizeof(element->len) + element->len;
			if(pdu_size < counter + len)
			{
				goto err;
			}
			memcpy(&element->data, &data->tx_status.ack, element->len);
			counter += len;
			break;
		case EVENT_EAPOL_TX_STATUS:
			printf("EVENT_EAPOL_TX_STATUS\n");
			/*dst*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			element->len = ETH_ALEN;
			len = sizeof(element->len) + element->len;
			...
			/*ack*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			element->len = sizeof(data->eapol_tx_status.ack);
			len = sizeof(element->len) + element->len;
			if(pdu_size < counter + len)
			{
				goto err;
			}
			memcpy(&element->data, &data->eapol_tx_status.ack, element->len);
			counter += len;
			break;
		case EVENT_EAPOL_RX:
			printf("EVENT_EAPOL_RX\n");
			/*src*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			element->len = ETH_ALEN;
			len = sizeof(element->len) + element->len;
			...
			memcpy(&element->data, data->eapol_rx.src, element->len);
			counter += len;
			/*data*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			element->len = data->eapol_rx.data_len;
			len = sizeof(element->len) + element->len;
			if(pdu_size < counter + len)
			{
				goto err;
			}
			memcpy(&element->data, data->eapol_rx.data, element->len);
			counter += len;
			break;
		...
		case EVENT_RX_MGMT:
			...
			/*datarate*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			element->len = sizeof(data->rx_mgmt.datarate);
			len = sizeof(element->len) + element->len;
			if(pdu_size < counter + len)
			{
				goto err;
			}
			memcpy(&element->data, &data->rx_mgmt.datarate, element->len);
			counter += len;	
			/*ssi_signal*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			element->len = sizeof(data->rx_mgmt.ssi_signal);
			len = sizeof(element->len) + element->len;
			if(pdu_size < counter + len)
			{
				goto err;
			}
			memcpy(&element->data, &data->rx_mgmt.ssi_signal, element->len);
			counter += len;	
			break;
		case EVENT_ASSOC:
			/*req_ies*/
...
int wpa_supplicant_data_parser(char *pdu, int p_size, union wpa_event_data *data, enum wpa_event_type *event)
{
	...
	switch(*event)
	{
		...
		case EVENT_TX_STATUS:
			...
			memcpy(&data->tx_status.type, &element->data, element->len);
			counter += len;
			/*dst*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			len = sizeof(element->len) + element->len;
			...
			/*data*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			data->tx_status.data_len = element->len;
			len = sizeof(element->len) + element->len;
			if(p_size < counter + len)
			{
				goto err; 
			...
			memcpy(&data->tx_status.stype, &element->data, element->len);
			counter += len;
			/*ack*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			len = sizeof(element->len) + element->len;
			...
			memcpy(&data->tx_status.ack, &element->data, element->len);
			counter += len;
			break;
		case EVENT_EAPOL_TX_STATUS:
			printf("EVENT_EAPOL_TX_STATUS\n");
			...
			p = malloc(element->len);
			memcpy(p, &element->data, element->len);
			data->eapol_tx_status.dst = (u8 *)p;
			counter += len;
			/*data*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			data->eapol_tx_status.data_len = element->len;
			len = sizeof(element->len) + element->len;
			if(p_size < counter + len)
			{
				goto err; 
			...
			memcpy(&data->eapol_tx_status.ack, &element->data, element->len);
			counter += len;
			break;
		case EVENT_RX_FROM_UNKNOWN:
			/*bssid*/
		...
		case EVENT_RX_MGMT:
			...
			/*data*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			data->rx_mgmt.frame_len = element->len;
			len = sizeof(element->len) + element->len;
			if(p_size < counter + len)
			{
				goto err;
			...
			memcpy(&data->rx_mgmt.datarate, &element->data, element->len);
			counter += len;
			/*ssi_signal*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			len = sizeof(element->len) + element->len;
			...
			memcpy(&data->rx_mgmt.ssi_signal, &element->data, element->len);
			counter += len;
			break;
		case EVENT_EAPOL_RX:
			printf("EVENT_EAPOL_RX\n");
			...
			data->eapol_rx.src = malloc(ETH_ALEN);
			memcpy((void *)data->eapol_rx.src, &element->data, ETH_ALEN);
			counter += len;
			/*data*/
			element = (struct wiflow_pdu_element *)(pdu + counter);
			data->eapol_rx.data_len = element->len;
			len = sizeof(element->len) + element->len;
			if(p_size < counter + len)
			{
				goto err; 
			}
			data->eapol_rx.data = malloc(element->len);
			memcpy((void *)data->eapol_rx.data, &element->data, element->len);
			counter += len;
			break;
		case EVENT_ASSOC:
			/*req_ies*/
