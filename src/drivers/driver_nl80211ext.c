...
void *global_ctx;
unsigned char own_addr[] = {0x0c, 0x82, 0x68, 0x28, 0xa3, 0xb8};
...
wpa_driver_nl80211_get_hw_feature_data(void *priv, u16 *num_modes, u16 *flags)
{
	...
	if(ret < 0)
	{
		...
		*num_modes = 2;
		*flags = 0;
		struct hostapd_hw_modes *hw_mode = (struct hostapd_hw_modes *)malloc(sizeof(struct hostapd_hw_modes));
		ret = remote_hw_modes_parser(buf, MAX_BUF_LEN, hw_mode);	
		if(ret < 0)
		{
			fprintf(stderr,"wiflow_pdu_parser Error,%s:%d\n",__FILE__,__LINE__);  
		}	
		return hw_mode;
	}
	return NULL;
...
static int wpa_driver_nl80211_sta_remove(void *priv, const u8 *addr)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211ext: wpa_sta_remove_format buf_size:%d",buf_size);
	/* send buf(argc) */
	ret = send(agentfd,buf,buf_size,0);
	if(ret < 0)
	{
		...
		size_t data_len, int encrypt, const u8 *own_addr, u32 flags)
{
	...
	if(ret < 0)
	{
		fprintf(stderr,"hapd_send_eapol_format Error,%s:%d\n",__FILE__,__LINE__);
		return -1;
	}
	wpa_printf(MSG_DEBUG, "nl80211ext: hapd_send_eapol_format buf_size:%d",buf_size);
	/* send buf(argc) */
	ret = send(agentfd,buf,buf_size,0);
	if(ret < 0)
	{
		...
		int idx, u8 *seq)
{
	...
	/* format  seq to buf */
	buf_size = MAX_BUF_LEN;
	ret = wpa_get_seqnum_format(buf, &buf_size, idx);
	if(ret < 0)
	{
