...
int main() 
{
    int i = 0;
    void * global_priv;
    /* init hapd,change it by your configure */
	...
	for (i = 0; wpa_drivers[i]; i++) 
	{
		...
		params.bssid = hapd.bssid;
		params.ifname = hapd.iface;
		params.ssid = hapd.ssid;
		params.ssid_len = strlen(hapd.ssid);       
		  ...
		  //   struct hostapd_frame_info *fi)
{
	...
	u16 fc, stype;
    /* what is it? */
    unsigned char da1[30]={0xb0,0x00,0x00,0x00,0x54,0xea,0xa8,0x16,0x18,0x90,0xc8,0x3a,0x35,0xc4,0x01,0xb8,0xc8,0x3a,0x35,0xc4,0x01,0xb8,0x00,
        0x00,0x00,0x00,0x02,0x00,0x00,0x00};
    memcpy(&da1[10],hapd->own_addr,ETH_ALEN);
    memcpy(&da1[16],hapd->own_addr,ETH_ALEN);
    /* what is it? */
    unsigned char da2[46]={0x10,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xc8,0x3a,0x35,0xc4,0x01,0xb8,0xc8,0x3a,0x35,0xc4,0x01,0xb8,0x00,
        0x00,0x11,0x04,0x00,0x00,0x01,0xc0,0x01,0x08,0x82,0x84,0x8b,0x96,0x0c,0x12,0x18,0x24,0x32,0x04,0x30,0x48,0x60,0x6c};
	memcpy(&da2[10],hapd->own_addr,ETH_ALEN);
    memcpy(&da2[16],hapd->own_addr,ETH_ALEN);
	...
	if(memcmp(hapd->bssid,mgmt->da,6)!=0)
	{	
		return;
	}
	...
	case WLAN_FC_STYPE_AUTH:
		...
		/* send_auth_reply */
		send_auth_reply(hapd,addr, hapd->bssid,0, 2, 0,NULL, 0);
